...
interface Building {
  ...
  idGrupo?: string; grupo?: { id: string; nombre: string }
}
interface Servicio  { id: string; idEdificio: string; nombreServicio: string; tipo: string; activo: boolean }
interface Propietario { id: string; nombre: string }
interface Grupo { id: string; nombre: string }
interface DepartamentoForm { nrDepartamento: string; piso: number; idPropietario?: string }
...
const EMPTY: Partial<Building> = {
  nombre: '', direccion: '', nroDepas: 0, cuentaBbva: '', cuentaBcp: '',
  serviciosActivos: {}, idGrupo: '',
}
...
export default function BuildingsPage() {
  ...
  const openNew = () => {
    // Admin: pre-seleccionar su grupo automáticamente
    const defaultGrupo = isAdministrador() ? (user?.idGrupo || '') : ''
    setEditing({ ...EMPTY, idGrupo: defaultGrupo })
    setServicios([]); setDeptos([]); setStep('edificio')
  ...
  const openEdit = async (b: Building) => {
    ...
    try {
      ...
      svcs.forEach((s: Servicio) => {
        normalizado[s.id] = !!(rawActivos[s.id] || rawActivos[s.tipo])
      })
  ...
  const save = async () => {
    ...
    if (!editing.idGrupo && isSupervisor()) return toast.error('Selecciona un grupo para el edificio')

    if (editing.id) {
      const habilitados = Object.values(editing.serviciosActivos || {}).filter(Boolean).length
      if (habilitados === 0 && servicios.length > 0) return toast.error('Habilita al menos un servicio')
    }
    ...
    try {
      const payload: any = {
        ...
        idGrupo: editing.idGrupo || undefined,
      }
      ...
      if (buildingId) {
        ...
        if (deptos.length > 0 && buildingId) {
          ...
          if (ok > 0) toast.success(`${ok} departamento${ok !== 1 ? 's' : ''} creado${ok !== 1 ? 's' : ''}`)
        }
      }
      await load(); close()
    ...
    <div style={{ padding:'2rem', maxWidth:1100, margin:'0 auto' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center',padding:'4rem',background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',color:'var(--text-muted)' }}>
          ...
          <p style={{ fontSize:'0.8rem',marginTop:'0.35rem' }}>Crea el primer edificio para comenzar</p>
        </div>
      ) : (
          ...
          {buildings.map(b => {
            ...
            return (
              <div key={b.id} style={{ background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:'1.5rem',position:'relative' }}>
                ...
                <div style={{ display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'0.75rem',paddingRight:'4.5rem' }}>
                ...
                {b.grupo && (
                  <div style={{ display:'flex',alignItems:'center',gap:'0.35rem',marginBottom:'0.6rem' }}>
                    ...
                    <span style={{ fontSize:'0.75rem',color:'var(--blue)',fontWeight:600 }}>{b.grupo.nombre}</span>
                  </div>
                )}
            ...
            <div style={{ overflowY:'auto',flex:1,padding:'1.5rem' }}>
              {(step === 'edificio' || !!editing.id) && (
                <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1rem' }}>
                  ...
                  {/* Selector de grupo — solo supervisor y solo al crear */}
                  {isSupervisor() && (
                    <Field label="Grupo *" span={2}>
                      <select value={editing.idGrupo || ''} onChange={e => setEditing({ ...editing, idGrupo: e.target.value })}
                        style={{ width:'100%',background:'var(--bg-elevated)',border:'1px solid var(--border)',color:'var(--text-primary)',borderRadius:'var(--radius)',padding:'0.5rem 0.75rem',fontSize:'0.875rem',fontFamily:'var(--font-body)' }}>
                  ...
                  {/* Admin: mostrar su grupo como info */}
                  {isAdministrador() && user?.idGrupo && (
                    <div style={{ gridColumn:'span 2',display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.5rem 0.75rem',background:'rgba(74,158,255,0.07)',border:'1px solid rgba(74,158,255,0.2)',borderRadius:'var(--radius)' }}>
                      <Layers size={14} color="var(--blue)" />
                      <span style={{ fontSize:'0.82rem',color:'var(--blue)',fontWeight:600 }}>
                        El edificio se asignará al grupo de tu cuenta
                      </span>
                    </div>
                  )}

                  <Field label="Nombre *" span={2}>
                    <input value={editing.nombre || ''} onChange={e => setEditing({ ...editing, nombre: e.target.value })} placeholder="Edificio Carlos Izaguirre" autoFocus
                      style={{ width:'100%',background:'var(--bg-elevated)',border:'1px solid var(--border)',color:'var(--text-primary)',borderRadius:'var(--radius)',padding:'0.5rem 0.75rem',fontSize:'0.875rem',fontFamily:'var(--font-body)' }} />
                  </Field>
                  <Field label="Dirección *" span={2}>
                    <input value={editing.direccion || ''} onChange={e => setEditing({ ...editing, direccion: e.target.value })} placeholder="Jr. Carlos Izaguirre 123, Lima"
                      style={{ width:'100%',background:'var(--bg-elevated)',border:'1px solid var(--border)',color:'var(--text-primary)',borderRadius:'var(--radius)',padding:'0.5rem 0.75rem',fontSize:'0.875rem',fontFamily:'var(--font-body)' }} />
                  </Field>
                  ...
                  <Field label="Cuenta BCP">
                    <input value={editing.cuentaBcp || ''} onChange={e => setEditing({ ...editing, cuentaBcp: e.target.value })} placeholder="191-12345678-0-56"
                      style={{ width:'100%',background:'var(--bg-elevated)',border:'1px solid var(--border)',color:'var(--text-primary)',borderRadius:'var(--radius)',padding:'0.5rem 0.75rem',fontSize:'0.875rem',fontFamily:'var(--font-body)' }} />
                  </Field>
                  <Field label="Cuenta BBVA" span={2}>
                    <input value={editing.cuentaBbva || ''} onChange={e => setEditing({ ...editing, cuentaBbva: e.target.value })} placeholder="0011-9234-56"
                      style={{ width:'100%',background:'var(--bg-elevated)',border:'1px solid var(--border)',color:'var(--text-primary)',borderRadius:'var(--radius)',padding:'0.5rem 0.75rem',fontSize:'0.875rem',fontFamily:'var(--font-body)' }} />
                  </Field>
                  ...
                  <div style={{ gridColumn:'span 2' }}>
                    ...
                    {loadingSvc ? (
                      ...
                      <div style={{ background:'var(--bg-elevated)',border:'1px solid rgba(74,158,255,0.15)',borderRadius:'var(--radius)',padding:'0.75rem 1rem' }}>
                  ...
                  <div style={{ display:'flex',flexDirection:'column',gap:'0.5rem' }}>
                    ...
                    {deptos.map((d, i) => (
                      <div key={i} style={{ display:'grid',gridTemplateColumns:'90px 60px 1fr 28px',gap:'0.5rem',alignItems:'center',background:'var(--bg-elevated)',borderRadius:'var(--radius)',padding:'0.5rem 0.6rem' }}>
                        <input value={d.nrDepartamento} onChange={e => updateDepto(i, 'nrDepartamento', e.target.value)} placeholder="201"
                          style={{ padding:'0.35rem 0.5rem',fontSize:'0.85rem',background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text-primary)',fontFamily:'var(--font-body)' }} />
                        <input type="number" min={1} value={d.piso} onChange={e => updateDepto(i, 'piso', parseInt(e.target.value) || 1)}
                          style={{ padding:'0.35rem 0.5rem',fontSize:'0.85rem',background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text-primary)',fontFamily:'var(--font-body)' }} />
                        <select value={d.idPropietario || ''} onChange={e => updateDepto(i, 'idPropietario', e.target.value)}
                          style={{ padding:'0.35rem 0.5rem',fontSize:'0.82rem',background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:4,color:'var(--text-primary)',fontFamily:'var(--font-body)' }}>
      ...
      {confirmDelete && (
        <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1100,padding:'1rem',backdropFilter:'blur(4px)' }} onClick={() => !deleting && setConfirmDelete(null)}>
          <div style={{ background:'var(--bg-surface)',border:'1px solid rgba(248,113,113,0.4)',borderRadius:'var(--radius-lg)',width:'100%',maxWidth:440,padding:'2rem',boxShadow:'var(--shadow-lg)' }} onClick={e => e.stopPropagation()} className="fade-up">
            ...
            <p style={{ fontSize:'0.875rem',marginBottom:'1.25rem' }}>
              Se eliminará <strong style={{ color:'#f87171' }}>{confirmDelete.nombre}</strong> y todos sus departamentos, servicios, mediciones y pagos.
            </p>
