...
interface Grupo       { id: string; nombre: string; edificios: { id: string; nombre: string }[] }
interface Building    { id: string; nombre: string }
interface Department  { id: string; nrDepartamento: string }

// Roles que el creador actual puede asignar
const ROLES_BY_CREATOR: Record<string, { value: string; label: string }[]> = {
  supervisor:    [
...
const EMPTY_FORM = {
  email:'', password:'', role:'propietario' as const,
  idGrupo:'', idEdificio:'', idDepartamento:'', idPropietario:'',
}
...
export default function UsersPage() {
  const { fmtDT } = useTz()
  const { user: currentUser, isSupervisor } = useAuthStore()
  ...
  const creatorRole = currentUser?.role || 'administrador'
  const availableRoles = ROLES_BY_CREATOR[creatorRole] || ROLES_BY_CREATOR.administrador

  // Edificios del grupo seleccionado
  const grupoEdificios = grupos.find(g => g.id === form.idGrupo)?.edificios || []
  ...
  // Al seleccionar departamento, buscar propietario vinculado
  useEffect(() => {
    ...
    api.get('/propietarios', { params: { buildingId: form.idEdificio } })
      .then(r => {
        const match = (r.data || []).find((o: any) => o.depto_id === form.idDepartamento)
        if (match) {
  ...
  const loadGrupos = async () => {
    try {
      if (isSupervisor()) {
        ...
        setGrupos(data.filter((g: any) => g.nombre !== 'SuperGrupo'))
      } else {
        // Administrador — solo su grupo
  ...
  const openEdit = (u: UserItem) => {
    setPropietarioDepto(null)
    setForm({
      email: u.email, password: '', role: u.role as any,
      idGrupo: '', idEdificio: u.idEdificio || '',
      idDepartamento: u.idDepartamento || '', idPropietario: u.idPropietario || '',
    })
    setEditing(u)
    setModal(true)
  }
  ...
  // Filtros disponibles según rol del usuario actual
    ...
    <div style={{ padding:'2rem', maxWidth:1100, margin:'0 auto' }}>
      ...
      {modal && (
        <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'1rem',backdropFilter:'blur(4px)' }} onClick={close}>
          <div style={{ background:'var(--bg-surface)',border:'1px solid var(--border-accent)',borderRadius:'var(--radius-lg)',width:'100%',maxWidth:500,maxHeight:'90vh',overflowY:'auto',boxShadow:'var(--shadow-lg)' }} onClick={e => e.stopPropagation()} className="fade-up">
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:'1rem',padding:'1.5rem' }}>
              ...
              <Field label="Rol *">
                <select style={inp} value={form.role} onChange={e => setForm({ ...form, role: e.target.value as any, idGrupo:'', idEdificio:'', idDepartamento:'' })}>
                  {availableRoles.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                </select>
              </Field>

              {/* Selector de grupo — visible para gestion y propietario */}
              {['gestion','propietario','administrador'].includes(form.role) && (
                <Field label="Grupo" hint="El grupo determina a qué edificios tiene acceso el usuario.">
                  <select style={inp} value={form.idGrupo} onChange={e => setForm({ ...form, idGrupo: e.target.value, idEdificio:'', idDepartamento:'' })}>
              ...
              {/* Selector de edificio — solo si hay grupo seleccionado */}
              {form.idGrupo && grupoEdificios.length > 0 && (
                <Field label="Edificio (opcional)">
                  <select style={inp} value={form.idEdificio} onChange={e => setForm({ ...form, idEdificio: e.target.value, idDepartamento:'' })}>
                    ...
                    {grupoEdificios.map(b => <option key={b.id} value={b.id}>{b.nombre}</option>)}
                  </select>
                </Field>
              )}

              {/* Selector de departamento — solo propietario con edificio */}
              ...
              {/* Info propietario del depto */}
              {form.role === 'propietario' && form.idDepartamento && (
                <div style={{ background:'var(--bg-elevated)',border:`1px solid ${propietarioDepto?'rgba(76,175,130,0.3)':'var(--border)'}`,borderRadius:'var(--radius)',padding:'0.65rem 1rem' }}>
                  ...
                  {propietarioDepto
                    ? <p style={{ fontWeight:600,color:'var(--green)' }}>✓ {propietarioDepto.nombre}</p>
                    : <p style={{ color:'var(--text-muted)',fontSize:'0.85rem' }}>⚠ Sin propietario asignado en el sistema</p>
                  }
                </div>
              )}
            </div>
