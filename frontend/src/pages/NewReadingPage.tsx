...
import {
  Droplets, Camera, Upload, CheckCircle2, Loader2, X, AlertTriangle,
  ArrowRight, RotateCcw, Lock,
} from 'lucide-react'
import ImageEditorModal from '../components/ImageEditorModal'
import MeterImageDisplay from '../components/MeterImageDisplay'
import {
  MESES_CORTO, TIPO_SERVICIO_CFG, getUnidadLabel, getReciboLabel, MODOS_CON_MEDICION,
} from '../constants'
...
const btn: React.CSSProperties = {
  ...
  color: '#0f1117', fontWeight: 600, fontSize: '0.875rem', padding: '0.65rem 1.2rem',
  borderRadius: 'var(--radius)', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-body)',
}
const btn2: React.CSSProperties = {
  ...btn, background: 'var(--bg-elevated)', color: 'var(--text-secondary)',
...
export default function NewReadingPage() {
  ...
  const [checkingMedicion, setCheckingMedicion] = useState(false)
  ...
  // ── Flag clave: ¿está bloqueado el ingreso? ──
  // Si ya existe medición para este recibo + depto, no se permite ingresar otra.
  const ingresoBloqueado = medicionExistente !== null
  ...
  // ── Verificar si ya existe medición para este recibo+depto ──
  useEffect(() => {
    if (!selectedDepto || !selectedRecibo) {
      setMedicionExistente(null)
      return
    }
    setCheckingMedicion(true)
    api.get('/readings', { params: { receiptId: selectedRecibo, deptId: selectedDepto } })
      .then(({ data }) => {
        setMedicionExistente(data && data.length > 0 ? data[0] : null)
      })
      .catch(() => setMedicionExistente(null))
      .finally(() => setCheckingMedicion(false))
  }, [selectedDepto, selectedRecibo])

  const validatePeriod = async () => {
    if (!selectedBuilding) return toast.error('Selecciona un edificio')
    try {
      const { data } = await api.get('/receipts/validate-period', {
      ...
      if (data.listo) {
        ...
        const rRes = await api.get('/receipts', {
          params: { serviceId: svcInicial.id, year: anio, month: mes },
        })
  ...
  const handleServicioChange = async (svcId: string) => {
    ...
    try {
      const rRes = await api.get('/receipts', {
        params: { serviceId: svcId, year: anio, month: mes },
      })
      setReceipts(rRes.data)
      if (rRes.data.length > 0) setSelectedRecibo(rRes.data[0].id)
  ...
  const uploadOcr = async () => {
    if (ingresoBloqueado) {
      toast.error('No se puede ingresar medición: ya existe una para este período')
      return
    }
    ...
    try {
      ...
      // El backend responde 409 con MEASUREMENT_ALREADY_EXISTS si alguien más
      // creó la medición mientras tanto (race condition o múltiples pestañas)
      if (e?.response?.status === 409) {
        toast.error('Ya existe una medición para este departamento en este período')
        // Refrescar el estado para mostrar el bloqueo
        const { data } = await api.get('/readings', {
          params: { receiptId: selectedRecibo, deptId: selectedDepto },
        })
        setMedicionExistente(data && data.length > 0 ? data[0] : null)
      } else {
        toast.error(e?.response?.data?.error || 'Error procesando imagen')
      }
    }
  ...
  const confirmOcr = async () => {
    ...
    try {
      await api.post('/readings/confirm-ocr', {
        sessionId: ocrResult.sessionId, idRecibo: selectedRecibo,
      ...
      if (e?.response?.status === 409) {
        toast.error('Ya existe una medición para este período. No se puede crear otra.')
        setStep('upload')
      } else {
  ...
  const confirmarManual = async () => {
    ...
    try {
      await api.post('/readings/confirm-ocr', {
        idRecibo: selectedRecibo, idDepartamento: selectedDepto,
    ...
    <div style={{ padding: '2rem', maxWidth: 820, margin: '0 auto' }}>
        ...
        {STEPS.map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1 }}>
            <div style={{
              ...
              border: `2px solid ${step === s ? 'var(--accent)' : STEPS.indexOf(step) > i ? 'var(--green)' : 'var(--border)'}`,
              background: step === s ? 'var(--accent)' : STEPS.indexOf(step) > i ? 'var(--green)' : 'var(--bg-elevated)',
              color: step === s ? '#0f1117' : STEPS.indexOf(step) > i ? 'var(--green)' : 'var(--text-muted)',
                  ...
                  return (
                    <div key={svc.id} onClick={() => handleServicioChange(svc.id)}
                      ...
                      <div style={{
                        ...
                        background: isSelected ? 'var(--green)' : 'transparent', flexShrink: 0,
      ...
      {step === 'upload' && (
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.75rem' }} className="fade-up">
          ...
          {/* ═════════════════════════════════════════════════════════════
              BLOQUEO PRINCIPAL: si ya hay medición, muestra esta tarjeta
              y oculta todo el resto del formulario
              ═════════════════════════════════════════════════════════════ */}
          {checkingMedicion && (
            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
              padding: '2rem', color: 'var(--text-muted)', fontSize: '0.875rem',
            }}>
              <Loader2 size={16} style={{ animation: 'spin 0.8s linear infinite' }} />
              Verificando si ya existe medición...
            </div>
          )}

          {!checkingMedicion && ingresoBloqueado && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: '1rem' }}>
                <Lock size={20} color="#a78bfa" style={{ flexShrink: 0, marginTop: 2 }} />
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 600, fontSize: '0.95rem', color: '#a78bfa', marginBottom: '0.4rem' }}>
                    Medición ya registrada para este período
                  </p>
                  <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: 1.5, margin: 0 }}>
                    Este departamento ya tiene una medición para el recibo seleccionado.
                    Si necesitas corregirla por algún motivo, contacta al soporte técnico.
                  </p>
                </div>
              </div>

              {/* Datos de la medición existente */}
              <div style={{
                ...
                <MeterImageDisplay
                  ...
                  alt="Foto de la medición registrada"
                  width={90}
                  height={70}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
                    {medicionExistente.lecturaActual != null && (
                      <span>
                        Lectura: <strong style={{ fontFamily: 'monospace', color: 'var(--text-primary)' }}>
                          {medicionExistente.lecturaActual} {unidad}
                        </strong>
                      </span>
                    )}
                    {medicionExistente.lecturaAnterior != null && (
                      <span>
                        Anterior: <strong style={{ fontFamily: 'monospace' }}>
                          {medicionExistente.lecturaAnterior} {unidad}
                        </strong>
                      </span>
                    )}
                    {medicionExistente.montoCalculado != null && (
                      <span>
                        Monto: <strong style={{ color: 'var(--accent)' }}>
                          S/. {parseFloat(medicionExistente.montoCalculado).toFixed(2)}
                        </strong>
                      </span>
                    )}
                  </div>
                  {medicionExistente.createdAt && (
                    <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.5rem', margin: 0 }}>
                      Registrada el {fmt(medicionExistente.createdAt, 'DD/MM/YYYY HH:mm')}
                    </p>
                  )}
                </div>
              </div>
          ...
          {/* ═════════════════════════════════════════════════════════════
              FORMULARIO de ingreso (solo visible si NO está bloqueado)
              ═════════════════════════════════════════════════════════════ */}
          {!checkingMedicion && !ingresoBloqueado && (
            <>
              ...
              <div onClick={() => setModoManual(m => !m)}
                ...
                <div style={{
                  ...
                  border: `2px solid ${modoManual ? 'var(--accent)' : 'var(--border)'}`,
                  background: modoManual ? 'var(--accent)' : 'transparent',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <>
                  <div
                    style={{
                      ...
                      gap: '0.75rem', cursor: 'pointer', background: 'var(--bg-elevated)',
                      minHeight: 180, marginBottom: '0.75rem', transition: 'border-color 0.2s',
                    }}
